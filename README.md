...
**Tournoi** injects each unordered pair of 2–4 contestants (active-world strains or saved organisms) in equal numbers into the start state. After the step budget, the larger living share wins; a gap of 10 % or less is a draw. Self-pairs are skipped. The matrix colours intensity by win margin; CSV lists per-pair wins, draws and mean shares (`src/sim/tournament.ts`).
