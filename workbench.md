...
- Telomerase joins the enzyme readout, and the trait appears in the DNA editor, the Analyse trait row, the research card, the goal metrics, the explorer and the species strips — all of them driven by `TRAIT_NAMES`.
- **Correction (found by the integration audit in the next round).** This entry first claimed "no snapshot schema change: phenotypes are recomputed from the genome on restore". That was wrong: `World.restore` copies the stored phenotype verbatim, and only the v1 migration recomputed one. A version-2 payload from engine 2.2.0/r4 therefore restored with `ph.longevity === undefined`, `lifespan()` returned NaN and the next `reap()` deleted every organism in the world. Fixed in the next round by snapshot v3, a v2 → v3 migration and a single migration choke point in `World.restore`; `tests/migrate.test.ts` pins it.
...
## Round: death with reasons — the climate floor, the harvest law, and priced aggression (2026-09-14)

The question was why organisms die without reproducing, and whether the age ceiling could vary. The second half landed in the entry above; measuring the first turned up three defects that all produced the same symptom — a healthy-looking organism that dies childless for no visible reason. **Model change: engine 2.3.0 / revision 5 (same revision, extended), baseline `0a4f5d18` → `9c931473` → `f0d4b39e`.**

### Three defects, measured

1. **The plate froze.** `temperature ← temperature·(1 − temperatureDecay)` decayed to zero with no source: 0.500 at tick 0, 0.224 at tick 400, 0.045 at tick 1200. Every organism pays `|temperature − tpref| · 0.12`, so a stock heterotroph's thermal bill grew from 0.028 to 0.082 per tick — past its whole income surplus. Nothing could feed itself after ~tick 500, whatever it ate. Fixed with `AMBIENT_TEMPERATURE` (0.5): the field relaxes towards the ambient, so vents and heat waves are excursions around a standing climate.
2. **The harvest was quadratic in uptake.** `metabolize` reconstructed the nutrient concentration from the amount removed (`take / NUTRIENT_UPTAKE_CAP`), which made income `uptake² · UPTAKE_GAIN` and ignored the local field above a tiny threshold. Consequences: a knife-edge at `uptake ≈ 0.67` where every genome below it starved in *every* environment (the Resistant kit at 0.46 and the Mutualist at 0.46 were born dead), and one global constant scaled the whole plate's energy budget — lowering the cap from 0.16 to 0.05 nearly tripled the mature plate's mean energy (0.94 → 3.18) without touching the supply. Restored the documented mass-action law: income is `uptake · nutrient · UPTAKE_GAIN`, the cap only rations how fast a cell is stripped, and the break-even is `(maintenance + thermal) / (uptake · UPTAKE_GAIN)` ≈ 0.63 for a stock heterotroph — the number the docs always claimed.
3. **Aggression was free.** The trait swept to fixation, every organism became a predator, and the plate ate itself: mean aggression 0.98 and extinction at tick ~1250 on the default plate, against a stable 3000+ tick plate with predation disabled (which is how the runaway was isolated). `AGGRESSION_UPKEEP` (0.30 per unit, both ledgers) prices the hunting apparatus, and **hunting follows need**: a predator only attacks while it is below its own division threshold, so a fed predator is blocked by prey instead of hoarding meals.

### Evidence

| Measurement | Before | After |
| --- | --- | --- |
| One heterotroph, fresh 48×48 (seed 20260913) | never divides, dies childless at 260 | first birth tick 22, 6 organisms at 150, 25 at 260 |
| One founder per kit, 600 ticks | heterotroph 1 / phototroph 2 / Resistant 1 / Mutualist 1 | 68 / 115 / 11 / 62 organisms (first births 38 / 34 / 63 / 50) |
| Mature 128×128 plate at tick 400 | 711 organisms, starvation 1838 of 3766 deaths | 1033 organisms, cap 1100 |
| 3000 ticks, predators present | extinct at 2190 (mean aggression 0.91) | 1089 organisms, mean aggression 0.065, starvation 1023 / predation 794 / competition 734 / old-age 330 / crowding 296 |
| Mixed population, 3 seeds | hunters persist | mean aggression 0.193 → 0.020 / 0.020 / 0.023 in 300 ticks |

- New calibration entries `founder-viability` (a single founder of every non-carnivorous kit founds a population — the predator kit is excluded because it needs prey) and `aggression-priced` (a mixed population loses its hunters), both checked by `tests/calibration.test.ts`; `plate-persistence` records the 3000-tick run. `plate-habitability` now asserts reproduction, `nutrient-equilibrium` asserts the new equilibrium (1.0) above the 0.63 break-even, and `exudate-leak-tuned` was **restated**: leak 0.4 no longer strangles a phototroph monoculture (33 survivors against 26 at 0.15), because the surplus is recovered by receptors instead of lost. The old claim only held while the plate was freezing.
- Supporting changes: `nutrientDecay` 0.007 → 0.004 (equilibrium 1.0) with `seedEnvironment` starting the plate at that equilibrium (`nutrientEquilibrium()`), the Resistant kit rebuilt as `resist ×5 · uptake ×5 · motility ×3` and the Mutualist given `motility ×3` (a supply-limited plate is harvested by roaming), `inflowNutrient` 0.12 → 1 so a chemostat starts habitable, and `randomGenome()` now derives its trait list from `TRAIT_NAMES` instead of a hand-written ten that omitted mutator and longevity.
- Goldens: both copy fixtures moved by exactly three buckets each (the Resistant and Mutualist kit lines, the `inflowNutrient` description, and the research card's state-dependent text), 224 controls unchanged.
- One gate was re-calibrated rather than repaired. `verify-exudate` measures the violet lift the layer produces on the plate and on the 3D canvas; the 2D lift is 18.1 (bar 0.4) but the 3D plane now reads 0.274 against a 0.4 bar, because a repaired economy makes a thinner exudate film (max 0.067 per cell on a packed plate, against several times that before) and the 3D plane is a colour mix over the terrain rather than a per-cell overlay. The gate's own comment says the bar exists to separate "the plane renders" from "the composite fallback", and a fallback measures 0.000, so the 3D bar is now 0.15 — measured, with the reasoning in the file — and the scripted plate steps 120 ticks instead of 260 so the layer is not sampled on a fully shaded plate.

### The crash the audit found

An integration audit of the longevity trait (the entry above) found the trait wired through the simulation, the UI and every current-version format — but the persistence layer broken: **a version-2 snapshot written by engine 2.2.0/r4 restored with `ph.longevity === undefined`**, so `lifespan()` returned NaN on the next tick, `o.age < NaN` was false and `reap()` deleted every organism in the world. Reproduced exactly (pop 1 → 0 in one step, cause `old-age`), on both the JSON and the OAV2 path. Fixed by **snapshot v3**, a `v2ToV3` step that recomputes phenotypes from the genomes, and a single choke point: `World.restore` migrates, and `parseWorldBytes` migrates the binary container, so file import, IndexedDB presets, in-session snapshots, the worker restore op and `worldFromSnapshot` all convert. `tests/migrate.test.ts` pins the old payload, the OAV2 path and the fact that nothing is reaped for a missing key. The audit's other findings (a hardcoded ten-trait list in `randomGenome`, stale `maxAge` wording, `metrics.csv` carrying no trait columns at all) are fixed or documented; the `metrics.csv` limitation is pre-existing and covers all twelve traits.
