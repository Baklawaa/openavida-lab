    ...
    <aside class="side" aria-label="${t("side.aria")}">
      ...
      <div class="side-scroll">
        ...
        <div id="panel-experiment" role="tabpanel" aria-labelledby="tab-experiment" hidden>
          ...
          <section class="block"><h2>${t("panel.data.title")}</h2><div class="row"><button id="btn-csv">${t("btn.csv")}</button><button id="btn-phylo">${t("btn.phylo")}</button><button id="btn-events">${t("btn.events")}</button><button id="btn-manifest">${t("btn.manifest")}</button><button id="btn-manifest-import">${t("btn.manifestImport")}</button><button id="btn-import">${t("btn.import")}</button></div><input id="import-file" type="file" accept="application/json,.json" hidden><input id="manifest-file" type="file" accept="application/json,.json" hidden><label class="toggle-row" id="opt-events"><span>${t("opt.events")}<small>${t("opt.events.hint")}</small></span><input type="checkbox"></label></section>
